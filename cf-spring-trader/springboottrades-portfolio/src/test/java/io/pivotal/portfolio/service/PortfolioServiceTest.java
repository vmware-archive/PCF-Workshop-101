...
public class PortfolioServiceTest {
	...
	public void doGetPortfolio() {
		...
		when(restTemplate.getForObject("http://quote-service/quote/{symbol}", Quote.class, ServiceTestConfiguration.quote().getSymbol())).thenReturn(ServiceTestConfiguration.quote());
	...
	public void doSaveOrder() {
		...
		when(restTemplate.getForEntity("http://account-service/accounts/{userid}/decreaseBalance/{amount}", Double.class, ServiceTestConfiguration.order().getAccountId(), amount )).thenReturn(response);
	...
	public void doSaveOrderSellOrder() {
		...
		//when(accountService.getUri()).thenReturn(uri);
		when(restTemplate.getForEntity("http://account-service/accounts/{userid}/increaseBalance/{amount}", Double.class, ServiceTestConfiguration.sellOrder().getAccountId(), amount )).thenReturn(response);
